...
class StateHandler extends EventHandler {
  ...
  onMessage(event) {
    ...
    if (this.isKickMessage(message)) {
      ...
      return this.minecraft.broadcastHeadedEmbed({
        message: `${user} foi expulso da guilda!`,
    ...
    if (this.isPromotionMessage(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `${username} foi promovido para ${newRank}`, color: '47F049' })
    }
    ...
    if (this.isDemotionMessage(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `${username} foi rebaixado para ${newRank}`, color: 'F04947' })
    }
    ...
    if (this.isRepeatMessage(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Você não pode dizer a mesma coisa duas vezes!`, color: 'DC143C' })
    }

    if (this.isNoPermission(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Eu não tenho permissão para fazer isso!`, color: 'DC143C' })
    }
    ...
    if (this.isOnlineInvite(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `${user} foi convidado para a guilda!`, color: '47F049' })
    }

    if (this.isOfflineInvite(message)) {
      let user = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[6].match(/\w+/g)[0]

      return this.minecraft.broadcastCleanEmbed({ message: `${user} foi convidado para a guilda enquanto offline!`, color: '47F049' })
    }
    ...
    if (this.isGuildMuteMessage(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `Chat da guilda foi mutado por ${time}`, color: 'F04947' })
    }

    if (this.isGuildUnmuteMessage(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Chat da guilda foi desmutado!`, color: '47F049' })
    }
    ...
    if (this.isUserMuteMessage(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `${user} foi mutado por ${time}`, color: 'F04947' })
    }

    if (this.isUserUnmuteMessage(message)) {
      let user = message.replace(/\[(.*?)\]/g, '').trim().split(/ +/g)[3]

      return this.minecraft.broadcastCleanEmbed({ message: `${user} foi desmutado!`, color: '47F049' })
    }

    if (this.isSetrankFail(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Cargo não encontrado.`, color: 'DC143C' })
    }

    if (this.isAlreadyMuted(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Esse jogador já está mutado!`, color: 'DC143C' })
    }

    if (this.isNotInGuild(message)) {
      let user = message.replace(/\[(.*?)\]/g, '').trim().split(' ')[0]

      return this.minecraft.broadcastCleanEmbed({ message: `${user} não está na guilda.`, color: 'DC143C' })
    }

    if (this.isLowestRank(message)) {
      let user = message.replace(/\[(.*?)\]/g, '').trim().split(' ')[0]

      return this.minecraft.broadcastCleanEmbed({ message: `${user} já está no cargo mais baixo!`, color: 'DC143C' })
    }

    if (this.isAlreadyHasRank(message)) {
      return this.minecraft.broadcastCleanEmbed({ message: `Jogador já tem esse cargo!`, color: 'DC143C' })
    }
    ...
    if (this.isPlayerNotFound(message)) {
      ...
      return this.minecraft.broadcastCleanEmbed({ message: `Jogador \`${user}\` não encontrado.`, color: 'DC143C' })
    }
    ...
    if (this.isOfficerMessage(message)) {
      ...
      let message2 = message.toString().replace(username, '').replace(guildRank, '').replace(/[\[\]]/g, '').replace('Officer >', '').replace('VIP+', '').replace('VIP', '').replace('MVP++', '').replace('MVP+', '').replace('MVP', '').replace(':', '').trim()
      return this.minecraft.broadcastOfficer({ username, message: message2, guildRank })
    }
